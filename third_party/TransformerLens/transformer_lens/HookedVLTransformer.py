...
import einops
import numpy as np
from transformer_lens.components.transformer_cross_block import (
...
class HookedVLTransformer(HookedRootModule):
    ...
    def input_to_embed(
        ...
        if len(tokens.shape) == 1:
            ...
            tokens = tokens[None]
        # if tokens.device.type != self.cfg.device: # CHANGE?
        ...
        with utils.LocallyOverridenDefaults(
            ...
            if return_type is None:
                ...
                if return_type == "logits":
                    return logits
                    del residual, cross_attention_states, shortformer_pos_embed, attention_mask, overriding_attention_mask
    ...
    def from_pretrained(
        ...
        n_devices: int = 1,
        device_list: Optional[List[int]] = None, # for specifying available GPU ids
        processor: Optional[AutoProcessor] = None,
        move_to_device: bool = True,
        ...
        **from_pretrained_kwargs,
        ...
        _mmp_src = None
        try:
            _mmp_src = hf_model.multi_modal_projector
        except AttributeError:
            try:
                _mmp_src = hf_model.model.multi_modal_projector
            except AttributeError:
                _mmp_src = None
        if _mmp_src is not None:
            multi_modal_device = device_list[0] if device_list else device # added for device_list support
            multi_modal_projector = _mmp_src.to(multi_modal_device)
        else:
            print("[HookedVLTransformer] WARNING: multi_modal_projector not found on hf_model")

        vision_device = device_list[0] if device_list else device # added for device_list support
        model = cls(
            cfg,
            vision_model=hf_model.vision_model.to(vision_device),
    ...
    def generate(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            batch_size, ctx_length = tokens.shape
            device = devices.get_device_for_block_index(0, self.cfg) if not hasattr(self.cfg, 'device_list') else self.device_list[0]
