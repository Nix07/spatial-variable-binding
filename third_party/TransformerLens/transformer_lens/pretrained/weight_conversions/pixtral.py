...
def convert_pixtral_weights(pixtral, cfg: HookedTransformerConfig):
    ...
    state_dict["embed.W_E"] = pixtral.language_model.embed_tokens.weight
    ...
    for l in range(cfg.n_layers):
        ...
        pos_embed = pixtral.language_model.rotary_emb(
        ...
        state_dict[f"blocks.{l}.ln1.w"] = pixtral.language_model.layers[
            l
        ].input_layernorm.weight

        W_Q = pixtral.language_model.layers[l].self_attn.q_proj.weight
        W_K = pixtral.language_model.layers[l].self_attn.k_proj.weight
        W_V = pixtral.language_model.layers[l].self_attn.v_proj.weight
        ...
        W_O = pixtral.language_model.layers[l].self_attn.o_proj.weight
        W_O = einops.rearrange(W_O, "m (n h)->n h m", n=cfg.n_heads)
        state_dict[f"blocks.{l}.attn.W_O"] = W_O

        state_dict[f"blocks.{l}.attn.b_O"] = torch.zeros(cfg.d_model, dtype=cfg.dtype)

        state_dict[f"blocks.{l}.ln2.w"] = pixtral.language_model.layers[
            l
        ].post_attention_layernorm.weight

        state_dict[f"blocks.{l}.mlp.W_in"] = pixtral.language_model.layers[
            l
        ].mlp.up_proj.weight.T
        state_dict[f"blocks.{l}.mlp.W_gate"] = pixtral.language_model.layers[
            l
        ].mlp.gate_proj.weight.T
        state_dict[f"blocks.{l}.mlp.b_in"] = torch.zeros(cfg.d_mlp, dtype=cfg.dtype)

        state_dict[f"blocks.{l}.mlp.W_out"] = pixtral.language_model.layers[
            l
        ].mlp.down_proj.weight.T
        state_dict[f"blocks.{l}.mlp.b_out"] = torch.zeros(cfg.d_model, dtype=cfg.dtype)

    state_dict["ln_final.w"] = pixtral.language_model.norm.weight

    state_dict["unembed.W_U"] = pixtral.lm_head.weight.T
