    ...
    def forward(
        ...
        if self.cfg.use_attn_in or self.cfg.use_split_qkv_input:
            ...
            if shortformer_pos_embed is not None:
                shortformer_pos_embed = repeat_along_head_dimension(
                    shortformer_pos_embed, n_heads=self.cfg.n_heads, clone_tensor=False # added for gpu usage?
                )
        else:
            attn_in = resid_pre
        ...
        if self.cfg.use_attn_in:
            attn_in = self.hook_attn_in(
                repeat_along_head_dimension(resid_pre, n_heads=self.cfg.n_heads, clone_tensor=False) # added for gpu usage?
            )
        ...
        if self.cfg.use_split_qkv_input:
            ...
            key_input = self.hook_k_input(
                repeat_along_head_dimension(resid_pre, n_heads=n_kv_heads, clone_tensor=False) # added for gpu usage?
            )
